...
test.describe('S02 策略下单与订单管理', () => {
  test('SCN-011 反转交易员在现价上方高抛开空｜LimitIncrease short @p0 @tx @serial', async ({}, testInfo) => {
    ...
    try {
      try {
        evidence = await runMarketFlow(runtime, { scenarioId: 'SCN-011', isLong: false, openTrigger: { orderType: 1, offsetPercent: 10 } });
      } catch (error) {
