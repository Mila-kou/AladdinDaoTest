...
test.describe('S07 订单类型全矩阵', () => {
  test('SCN-066 空头交易员下跌止盈，补齐 LimitDecrease short @p0 @tx @serial', async ({}, testInfo) => {
    // 市价开空 → 挂 TP（LimitDecrease short，P−10%）→ 推价下穿触发 → Keeper 执行止盈全平。
    test.setTimeout(360_000);
    test.skip(testInfo.project.name !== 'tx-fork', 'SCN-066 在 tx-fork 执行（06-策略下单矩阵）');
    const runtime = await applyCaseTrader(loadRuntimeConfig(), 'SCN-066');
    ...
    try {
      try {
        evidence = await runMarketFlow(runtime, { scenarioId: 'SCN-066', isLong: false, closeTrigger: { orderType: 3, offsetPercent: -10 } });
      } catch (error) {
      ...
      const evidencePath = testInfo.outputPath('scn-066-evidence.json');
      await writeFile(evidencePath, stringifyEvidence(evidence), 'utf8');
      await testInfo.attach('scn-066-evidence.json', {
        path: evidencePath,
      ...
      testInfo.annotations.push({
        type: 'check-result',
        description: `链上 ${evidence.assertions.length} 项核对通过（LimitDecrease short 下跌止盈）；已记录用户签名挂单/平仓、推价触发与 Keeper 执行完整回执。`,
      });
    } finally {
