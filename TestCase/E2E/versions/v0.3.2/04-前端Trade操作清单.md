...
本节结论固定到 [`CURRENT.json`](../../../../Docs/contract-releases/CURRENT.json) 指向的合约与 apps 提交，不用需求文档日期替代源码事实：
...
详细首次建连、续期、存储、Permit、Cancel All/Protection Roll 等动作覆盖和 v0.3.2 当前 GAP 见 [Standard-Relay-Flash-OneClick-(v0.3.2).md](Standard-Relay-Flash-OneClick-(v0.3.2).md)。
...
完整原子用例见 [Trade-测试用例矩阵.md](Trade-测试用例矩阵.md) C2 与 §5 C 的 FT/XT-FLASH-ROLL 专项。
