...
test.describe('S02 策略下单与订单管理', () => {
  test('SCN-011 反转交易员在现价上方高抛开空｜LimitIncrease short @p0 @tx @serial', async ({ page }, testInfo) => {
    ...
    try {
      try {
        evidence = await runMarketFlow(runtime, withUiHooks({ scenarioId: 'SCN-011', isLong: false, openTrigger: { orderType: 1, offsetPercent: 10 } }, uiHooks));
      } catch (error) {
      ...
      const evidencePath = testInfo.outputPath('scn-011-evidence.json');
      await writeFile(evidencePath, stringifyEvidence(evidence), 'utf8');
      await testInfo.attach('scn-011-evidence.json', {
        path: evidencePath,
      ...
      testInfo.annotations.push({
        type: 'check-result',
        description: `链上 ${evidence.assertions.length} 项核对通过（LimitIncrease short 挂单/触发/执行链路）；已记录用户签名挂单/平仓、推价触发与 Keeper 执行完整回执。`,
      });
    } finally {
