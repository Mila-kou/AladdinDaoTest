...
// → ③ Mock Market Bundle 初始化 → ④ Mock Oracle 价格 → ⑤ 环境检查（验收）。
export function renderEnvironmentsHtml(generatedAt: string): string {
  ...
  <section class="panel section-card" id="oracle-panel">
    <div class="section-heading"><div><h2>④ Mock Oracle 价格</h2><p>default-mock 两个 Oracle 的链上实时读数。Fork 新区块使用真实时钟，价格时间戳停在上次设价时刻——隔天执行订单会因价格过期取消，跑批次前先刷新时间戳。</p></div><button id="refresh-oracle-read" type="button" class="secondary">重新读取</button></div>
    <div id="oracle-cards" class="field-grid"></div>
    <div class="init-actions">
      ...
      <label>新价格（USD 十进制）<input id="oracle-price" type="text" inputmode="decimal" autocomplete="off" placeholder="例如 60000"></label>
      <button id="set-oracle-price" type="button" class="secondary">设置新价格</button>
    </div>
    <p id="oracle-status" class="notice"></p>
  </section>
  ...
  const script = `
    ...
    function renderEnvironmentMode(){const supported=state.configuration.capabilities.initializesDefaultMockResources;const isBase=environment.value==='base-sepolia';document.getElementById('initialization-panel').hidden=!supported;document.getElementById('oracle-panel').hidden=!supported;document.getElementById('create-tenderly-fork').hidden=!supported;document.getElementById('fork-guide').hidden=!supported;document.getElementById('rpc-step-description').textContent=isBase?'Base Sepolia 使用公共 RPC：填好主 RPC/WSS 后保存即可。':'先有可用的 Fork，再谈其他配置。填好 RPC 后点击保存即可生效。';document.getElementById('configuration-description').textContent=isBase?'Base Sepolia 使用已有部署：这里只维护 Trade、Trader、Admin、签名和本机数据源。默认折叠，需要修改自行展开。':'默认折叠：不修改即沿用现有配置；需要调整时展开对应分组。';document.getElementById('check-description').textContent=isBase?'只读检查 Trade、Base Sepolia RPC、部署清单、账户/Admin 与 Case 数据源；不会部署 Mock Token/Oracle、Market，也不会执行 Keeper 初始化。':'只读检查 Trade、RPC、部署清单、账户、角色、default-mock 与 Case 数据源——配置完成后的验收步骤。';}
    ...
    function renderOraclePrices(result){document.getElementById('oracle-cards').innerHTML=result.oracles.map(function(oracle){const hours=(oracle.ageSeconds/3600).toFixed(1);const freshness=oracle.fresh?'<span class="status PASS">新鲜 · '+oracle.ageSeconds+'s</span>':'<span class="status FAIL">已过期 '+hours+' 小时</span>';return '<article class="field"><span class="field-line"><strong>'+(oracle.role==='index'?'Index Oracle':'USDC Oracle')+' · '+esc(oracle.description)+'</strong>'+freshness+'</span><small><code>'+esc(oracle.address)+'</code></small><div>价格 <strong>'+esc(oracle.priceDisplay)+' USD</strong>（raw '+esc(oracle.priceRaw)+' · '+oracle.decimals+' 位小数）</div><div class="muted">时间戳 '+oracle.updatedAt+'（'+esc(oracle.updatedAtIso)+'）</div></article>';}).join('');}
    async function loadOraclePrices(){const panel=document.getElementById('oracle-panel');if(panel.hidden)return;const statusEl=document.getElementById('oracle-status');statusEl.textContent='正在读取 Mock Oracle 链上状态…';try{const result=await requestJson('/api/mock-oracle-prices?environment='+encodeURIComponent(environment.value));renderOraclePrices(result);statusEl.textContent='链上实时读数 · 最新区块 #'+result.latestBlock.number+'（'+esc(result.latestBlock.timestampIso)+'）。数值不落盘：mock-resources.json 只登记初始化值。';}catch(error){document.getElementById('oracle-cards').innerHTML='';statusEl.textContent='读取失败：'+error.message;}}
    async function oracleAction(body,button){button.disabled=true;const statusEl=document.getElementById('oracle-status');statusEl.textContent='正在发送环境管理交易（admin RPC 免签名）…';try{const result=await requestJson('/api/mock-oracle-prices',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.assign({environment:environment.value},body))});statusEl.textContent='完成：'+result.operations.map(function(op){return op.role+' '+op.txHash.slice(0,12)+'…（价格 '+op.after.priceDisplay+' USD，时间戳 '+op.after.updatedAt+'）';}).join('；');await loadOraclePrices();}catch(error){statusEl.textContent='操作失败：'+error.message;}finally{button.disabled=false;}}
    async function load(){document.getElementById('connection').textContent='正在读取 '+environment.value+'…';const query=encodeURIComponent(environment.value);const configurationResult=await requestJson('/api/environment-configuration?environment='+query);state.configuration=configurationResult.configuration;state.profile=null;state.marketSources=[];renderConfiguration();renderEnvironmentMode();if(state.configuration.capabilities.initializesDefaultMockResources){state.profile=await requestJson('/api/environment-initialization-profile?environment='+query);renderProfile();try{await loadMarketSources();}catch(error){state.marketSources=[];renderMarketSources();document.getElementById('market-source-status').textContent='Market 列表读取失败：'+error.message;}loadOraclePrices().catch(function(){});}document.getElementById('connection').textContent=environment.value==='base-sepolia'?'已连接 Base Sepolia 配置；该环境不会加载或执行 Mock/Market/Keeper 初始化。':'已连接本机服务；RPC 显示完整地址，私钥和 Token 只显示配置状态。按 ①→⑤ 顺序完成配置。';}
    ...
    document.getElementById('refresh-oracle-read').addEventListener('click',function(){loadOraclePrices().catch(function(){});});
    ...
    document.getElementById('set-oracle-price').addEventListener('click',function(){const price=document.getElementById('oracle-price').value.trim();if(!/^(?:0|[1-9]\\d*)(?:\\.\\d{1,18})?$/.test(price)||Number(price)<=0){document.getElementById('oracle-status').textContent='新价格必须是大于 0 的十进制数字。';return;}oracleAction({action:'set-price',target:document.getElementById('oracle-target').value,price:price},this);});
  ...
  return renderPageShell({
    title: '测试环境',
    subtitle: '按 ①Fork 与 RPC → ②Trade/账户配置（默认折叠）→ ③Mock 初始化 → ④Oracle 价格 → ⑤环境检查 的顺序完成配置',
    active: 'environments',
