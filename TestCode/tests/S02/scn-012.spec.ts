...
test.describe('S02 策略下单与订单管理', () => {
  test('SCN-012 趋势交易员突破追多｜StopIncrease long @p0 @tx @serial', async ({}, testInfo) => {
    ...
    try {
      try {
        evidence = await runMarketFlow(runtime, { scenarioId: 'SCN-012', isLong: true, openTrigger: { orderType: 6, offsetPercent: 10 } });
      } catch (error) {
