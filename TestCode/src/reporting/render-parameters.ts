...
export function renderParametersHtml(reference: ParameterReference, generatedAt: string): string {
  ...
  <section class="panel action-panel" id="oracle-panel" hidden>
    ...
    <div class="action-row">
      ...
      <button id="set-oracle-price" type="button">设置新价格</button>
    </div>
  ...
  const script = `(function(){
    ...
    function renderOraclePrices(result){document.getElementById('oracle-cards').innerHTML=result.oracles.map(function(oracle){const hours=(oracle.ageSeconds/3600).toFixed(1);const freshness=oracle.fresh?'<span class="chip chip-fresh">新鲜 · '+oracle.ageSeconds+'s</span>':'<span class="chip chip-stale">已过期 '+hours+' 小时</span>';return '<article class="oracle-card"><div class="oracle-card-head"><strong>'+(oracle.role==='index'?'Index Oracle':'USDC Oracle')+' · '+escapeHtml(oracle.description)+'</strong>'+freshness+'</div><code>'+escapeHtml(oracle.address)+'</code><div>价格 <strong>'+escapeHtml(oracle.priceDisplay)+' USD</strong>（raw '+escapeHtml(oracle.priceRaw)+' · '+oracle.decimals+' 位小数）</div><div class="muted">时间戳 '+oracle.updatedAt+'（'+escapeHtml(oracle.updatedAtIso)+'）</div></article>';}).join('');}
