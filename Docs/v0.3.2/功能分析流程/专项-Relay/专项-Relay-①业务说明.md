...
| 实测补充（2026-09-06） | 余额落在「上限～上限 + 1 USDC」这段死区时，点 Max 没反应，按钮上显示的是「Enter Amount」，像是你还没输金额；其实这时手输一个小于「余额 − 上限」的金额是能成交的。切到 Standard 后，这条提示和切换按钮会一起消失，而 Standard 下 Max 仍会预留 1 USDC，死区变成「余额 ≤ 1 USDC」。手机端开仓面板根本没有「Switch to Standard」按钮 |
...
| Enter Amount（点了 Max 之后仍显示） | 余额落在死区，Max 算出 0，不是你没输金额 | 手输一个小一点的金额，或充值到「上限 + 1 USDC」以上 |

### 三种「卡住」怎么分

| | 跑腿费不够 | Max 死区 | 撮合员跳单 |
|---|---|---|---|
| 什么时候发生 | 提交**前** | 点 Max 时 | 提交**后** |
| 链上有没有交易 | 没有 | 没有 | **有**，订单已创建 |
| 页面表现 | Insufficient USDC for Relay Fee + 切换按钮 | 输入框没反应，按钮「Enter Amount」 | 订单一直「已创建」不成交 |
| 自救 | 充值到 ≥ 上限 | 手输金额，或充值到 > 上限 + 1 USDC | 撤单退款后重发 |

前两种根本没发交易，第三种发了但没人执行。「链上有没有订单已创建」是硬分界，别把撮合员的问题误报成余额问题。
