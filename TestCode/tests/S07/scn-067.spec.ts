...
test.describe('S07 订单类型全矩阵', () => {
  test('SCN-067 空头交易员上涨止损，补齐 StopLossDecrease short @p0 @tx @serial', async ({}, testInfo) => {
    // 市价开空 → 挂 SL（StopLossDecrease short，P+10%）→ 推价上穿触发 → Keeper 执行止损全平。
    test.setTimeout(360_000);
    test.skip(testInfo.project.name !== 'tx-fork', 'SCN-067 在 tx-fork 执行（06-策略下单矩阵）');
    const runtime = await applyCaseTrader(loadRuntimeConfig(), 'SCN-067');
    ...
    try {
      try {
        evidence = await runMarketFlow(runtime, { scenarioId: 'SCN-067', isLong: false, closeTrigger: { orderType: 4, offsetPercent: 10 } });
      } catch (error) {
      ...
      const evidencePath = testInfo.outputPath('scn-067-evidence.json');
      await writeFile(evidencePath, stringifyEvidence(evidence), 'utf8');
      await testInfo.attach('scn-067-evidence.json', {
        path: evidencePath,
      ...
      testInfo.annotations.push({
        type: 'check-result',
        description: `链上 ${evidence.assertions.length} 项核对通过（StopLossDecrease short 上涨止损）；已记录用户签名挂单/平仓、推价触发与 Keeper 执行完整回执。`,
      });
    } finally {
