...
- tx-fork 是 Tenderly 私有 fork，链 ID 为独立的 **99911**（部署基线 Base Sepolia 为 84532）；核对 chainId 以 `TestCode/config/mock-resources.json` 登记为准。tx-fork/oracle-fork fork 自 v0.3.1 部署，time-fork 99913 待建（`TestCode: npm run env:vnet:create -- --env time-fork` 一键创建固定 Chain ID 的 Virtual TestNet，需 Tenderly Access Token；CURRENT.json 登记 pending）——批次 `release` 标签反映环境实际版本，与 CURRENT.json 目标版本不一致时不充当目标版本回归材料。
