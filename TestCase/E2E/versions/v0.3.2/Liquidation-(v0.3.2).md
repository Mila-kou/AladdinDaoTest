...
关键差异：清算判定**不计 Liquidation Fee**，执行结算**计 Liquidation Fee**；清算不允许负 dynamicSpread。正 raw 只有在 effective MAX 大于 0 时才得到正 final，并仍受 MAX clamp。
...
因此 raw spread 为负时 final 至少为 0；raw 为正且 `effectiveMax>0` 时可形成不利成交价（区间内保留、越界钳到 effectiveMax），`effectiveMax=0` 时正 raw 也会落为 0。long 清算按 sell 路径使用 `oracle.min × (1-d)` 向下取整；short 清算按 buy 路径使用 `oracle.max × (1+d)` 向上取整。
...
偿付足够时，pool 部分从 PositionVault 转入 Market Vault，receiver 部分增加 `LIQUIDATION_FEE_TYPE` claimable；余数归入 pool 部分。claimable 不是领取终点：`FeeHandler.claimFees` 会将该 market/token/type 的 claimable 清零、从 PositionVault 转入 FeeHandler available fee；随后 `FEE_KEEPER.withdrawFees` 清零 available，并把 token 转给配置的 `FEE_RECEIVER`（或调用其 reward-splitter 回调）。用例必须核对 claimable、available、PositionVault、FeeHandler 与最终 receiver 五段，而不能停在“claimable 已增加”。

清算 Close/Position Fee 仍执行 Referral 分解。偿付充分且 affiliate reward 非零时，`handleReferral` 会增加 `affiliateRewardKey(marketIndex, token, affiliate)` 及 pool 汇总；affiliate 后续通过 `claimAffiliateRewards` 从 PositionVault 转给指定 receiver。该路径与 `LIQUIDATION_FEE_TYPE` 的 FeeHandler 领取不是一回事。insolvent empty-fees 路径的 affiliate amount 可为 0，必须以实际 fees/event/key 为准。

其余成本与 Close Fee、Funding、PnL、抵押物共同进入清算结算。清算判定不因 Liquidation Fee 而提前触线，但执行后的用户残值会受它影响。insolvent 早退时费用整包可能不进入 claimable；此时不能断言“计算出 Liquidation Fee 就一定完成分账”。
...
| Liquidation Fee 领取链 | `LIQUIDATION_FEE_TYPE` claimable → FeeHandler available → `FEE_KEEPER` withdraw → `FEE_RECEIVER`/reward splitter |
| Referral/affiliate | 偿付充分且 reward 非零时增加 affiliateReward key；affiliate 领取后从 PositionVault 转给指定 receiver |
| PositionVault / Market Vault / FeeHandler / claimable / receiver | 按 PnL、Funding、费用和实际可支付额变化 |
| output/residual collateral | 可支付给用户的剩余金额；资不抵债时可能为 0 |
| 内部 Order 与事件 | OrderCreated、执行/移除、PositionFees、PositionDecrease 等形成清算证据链 |

### 5.2 间接影响

- 全平后自动取消该 positionKey 下登记为 auto-cancel 的 TP/SL。每个被取消订单先从自己的 execution fee 支付本次取消 Keeper，只有 surplus 才退给 `cancellationReceiver`/receiver；应分别核对 Keeper execution-fee 支付、`ExecutionFeeRefund`（surplus>0）、OrderVault 与接收方余额，不能断言原 execution fee 全额返还。
...
- 每个 early-return step 都发 `PositionFeesInfo`，但内容不是一律相同：Funding/PnL step 传入当时计算出的理论 fee struct（不是实际已收凭证，且源码提示 funding/fee-for-pool 可能不准确）；Fee step 在不足后先调用 `getEmptyFees`，所以 Info 中普通/negative/liquidation 费用已清零，只保留 positive Funding、latest indices 等字段。
...
3. `PositionFeesCollected` 始终发出；insolvent 早退还会额外发 `PositionFeesInfo` 与 `InsolventClose`。Funding/PnL step 的 Info 是当时理论结构，Fee step 的 Info 已经过 `getEmptyFees`；均须与实际不足/余额事件交叉核对。
...
| Referral | 无 referral/有 referral；affiliate reward=0/1/普通值；领取前后 | 清算仍按实际 fees 累计 affiliate reward；领取清零对应 key 并从 PositionVault 转给指定 receiver；insolvent empty-fees 路径不虚增 |
| Liquidation Fee 领取 | claimable=0/1/普通值；claim 重复；非/正确 FEE_KEEPER withdraw | claimable→FeeHandler available→FEE_RECEIVER 守恒；重复不多领；withdraw 权限正确 |
| collateral 脱锚 | min<max 与不同 decimals | remaining、应付/应收和 fee amount 使用规定价格侧 |
| 资不抵债 | 刚好够成本、差 1 token-unit、严重不足；分别命中 funding/pnl/fees 早退 | 清算全平仍可完成适用路径；Funding/PnL Info 只作理论结构，Fee-step Info 已清空相应费用；实际用 Insufficient/Collected/InsolventClose 与余额差取证，不虚构坏账 ledger |
| TP/SL | 0、1、最大允许 auto-cancel 订单；execution fee 刚好/有 surplus | 清算后全部按列表取消；每单先付取消 Keeper、仅 surplus 退款，OrderVault 与 receiver/keeper 差分守恒 |
...
| `CT-LIQ-FEE-005` | Liquidation Fee 公式、pool/claimable 分账与 FeeHandler→FEE_RECEIVER 领取终点 |
| `CT-LIQ-REF-006` | 清算 Referral/affiliate reward 累计、领取与 insolvent empty-fees 对照 |
...
每个成功数据集至少保存：Position 与 OI 前态、Grace、Oracle、配置、Keeper 参数、Reader 持久化状态预检三项数值、链上最终判定、内部 orderKey、执行价/final spread、capped/uncapped PnL、Funding、费用、PositionVault/Market Vault/FeeHandler/claimable/available/FEE_RECEIVER/affiliate/用户余额、auto-cancel 每单的 Keeper 支付与 surplus 退款、订单和页面终态。失败数据集保存 revert 原因及全状态不变证据。
