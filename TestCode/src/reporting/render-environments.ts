...
export function renderEnvironmentsHtml(generatedAt: string): string {
  ...
  const script = `
    ...
    async function startWizard(){
      ...
      if(picks.deploy&&!wizardDryRun)message+='\\n注意：部署会更新 config/environment-bindings.json 与 CURRENT.json 登记。';
