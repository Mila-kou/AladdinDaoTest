...
- `allowNegativeSpread=false`，effective minimum spread 至少为 0；effective maximum 随后被抬到不低于 effective minimum。正 raw 不会被特殊规则直接归零，但只有 `effectiveMax>0` 时 final 才可能为正；`0<raw<=effectiveMax` 时保留 raw，`raw>effectiveMax` 时钳到 effectiveMax。
...
5. full ADL 在 Funding、PnL 或 Fee 任一阶段仍有未付成本时可进入 insolvent early return；每种成功早退都发 `PositionFeesInfo` 和 `InsolventClose(step)`，但事件中的 fees 不同：Funding/PnL 阶段传入当时的理论 fees，Fee 阶段先 `getEmptyFees`，因此 Info 已清零普通/negative/liquidation 费用，只保留 positive Funding 与 latest indices 等字段。只有 Funding 不足分支必有 `InsufficientFundingFeePayment`。最终 `PositionFeesCollected` 的费用字段也可能被清零，不能无条件断言“Close Fee 已收取”。
...
| dynamicSpread | `effectiveMin=0`；effectiveMax=0/>0；raw<0、=0、0<raw≤max、raw>max | 负值/零值 final=0；effectiveMax=0 时正 raw 也钳为 0；effectiveMax>0 时正 raw 分别保留或钳到正 max |
...
| `CT-ADL-004` | raw 负/正与 effective MAX=0/>0 时的 final clamp、执行价、PnL/OI/事件 |
