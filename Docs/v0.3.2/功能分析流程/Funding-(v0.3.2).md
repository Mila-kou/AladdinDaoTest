...
| `PositionFeesCollected` / `PositionFeesInfo` | 仓位实际/早退 Funding 分解；Funding/PnL step 的 Info 使用当时理论 fees，Fee step 的 Info 已经过 `getEmptyFees`，不能把所有 Info 都当成完整理论值 |
...
| 资不抵债 | collateral 不足覆盖 negative Funding；普通 Increase/Decrease（含普通全平）对比 full Liquidation/full ADL | 普通仓位动作回退，Funding 状态、仓位、余额及不足事件均不留存；只有 full Liquidation/full ADL 可成功 early return，记录 expected/actual、未支付差额与 step。协议没有独立坏账/insurance ledger，不得把差额命名为已入账坏账 |
