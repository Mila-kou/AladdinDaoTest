...
2. **跑用例**：跑批页选环境与 Keeper 模式；或 npm scripts 直跑。模式选择与 spec 保护差异见 `references/run-and-verify.md`。一键链路入口：`npm run pipeline -- --env tx-fork --cases SCN-009,SCN-022`（创建 fork→初始化→验收→跑批→看板回收/归档，先 `--dry-run` 看计划；flags 与退出码见 TestCode/README「pipeline 一键链路」；`--per-case-traders` 按例专属 Trader，`--ui` 前端通道一键化（互斥、仅 tx-fork、自动过滤为 9 条支持 spec、退出码 35=前置未就绪））。
