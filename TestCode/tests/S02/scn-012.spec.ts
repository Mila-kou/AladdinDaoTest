...
test.describe('S02 策略下单与订单管理', () => {
  test('SCN-012 趋势交易员突破追多｜StopIncrease long @p0 @tx @serial', async ({ page }, testInfo) => {
    // 突破追多（P+10% 上破）：StopIncrease 挂单 → 推价上破触发 → Keeper 执行 → 全平。
    test.setTimeout(360_000);
    test.skip(testInfo.project.name !== 'tx-fork', 'SCN-012 在 tx-fork 执行（06-策略下单矩阵）');
    const runtime = await applyCaseTrader(loadRuntimeConfig(), 'SCN-012');
    ...
    try {
      try {
        evidence = await runMarketFlow(runtime, withUiHooks({ scenarioId: 'SCN-012', isLong: true, openTrigger: { orderType: 6, offsetPercent: 10 } }, uiHooks));
      } catch (error) {
      ...
      const evidencePath = testInfo.outputPath('scn-012-evidence.json');
      await writeFile(evidencePath, stringifyEvidence(evidence), 'utf8');
      await testInfo.attach('scn-012-evidence.json', {
        path: evidencePath,
      ...
      testInfo.annotations.push({
        type: 'check-result',
        description: `链上 ${evidence.assertions.length} 项核对通过（StopIncrease long 突破追单链路）；已记录用户签名挂单/平仓、推价触发与 Keeper 执行完整回执。`,
      });
    } finally {
