...
v0.3.2 合约要求 `SubaccountApproval` 包含命名 `slot`，但 CURRENT 固定前端的类型、typed data 与 ABI 尚未完整包含该字段，因此当前 1CT 功能层应标为 `GAP`，版本前端准入另保持 `NOT_READY`；不能因为 UI 可选就判定可用。完整切换、会话、动作和缺口见 [Standard-Relay-Flash-OneClick-(v0.3.2).md](Standard-Relay-Flash-OneClick-(v0.3.2).md)。
...
完整公式、适用范围、零值死区、精度与报价状态表见 [Standard-Relay-Flash-OneClick-(v0.3.2).md](Standard-Relay-Flash-OneClick-(v0.3.2).md#721-relay1ct-max-计算)。
