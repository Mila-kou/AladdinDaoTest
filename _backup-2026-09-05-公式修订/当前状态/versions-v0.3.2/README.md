# FX100 v0.3.2 Trade 功能需求与测试主目录

> 基线以 [`Docs/contract-releases/CURRENT.json`](../../../../Docs/contract-releases/CURRENT.json) 为唯一事实源。本目录不把历史 v0.3.1 PASS 继承为 v0.3.2 PASS。

## 1. 目的

本目录先说明 v0.3.2 Trade **是什么、用户能做什么、合约如何处理、前端应如何表现**，再把功能需求映射到测试用例。它不是脚本索引，也不以脚本执行速度代表功能覆盖完整度。

测试结论分三层记录：

1. **CT（Contract Trade）**：合约交易、定价、账本、事件与错误。
2. **FT（Frontend Trade）**：Trade 页面输入、预览、签名、状态反馈与展示。
3. **XT（Cross-layer Trade）**：同一笔交易的页面、请求参数、交易回执、事件、Reader 和余额差分一致性。

任何一层通过都不能代替另外两层。特别是 TestCode 中使用私钥直接签名、Inline Keeper 或 Driver 构造交易的快速脚本，只能证明它实际断言到的链上链路；未经过页面填写、按钮点击和钱包交互时，不得标记 FT 通过。

## 2. 推荐阅读顺序

| 文件 | 用途 |
|---|---|
| [cases/IT-VAULT-MARKET.md](cases/IT-VAULT-MARKET.md) | v0.3.2 的 4 条 Vault/Market 域级集成用例 |
| [ContractCodeSummary v0.3.2](../../ContractCodeSummary/v0.3.2/README.md) | v0.3.2 公式、模块与参数定义 |
| [00-Trade功能总览.md](00-Trade功能总览.md) | Trade 边界、参与者、功能域和总流程 |
| [01-Order字段与状态.md](01-Order字段与状态.md) | v0.3.2 Order 全字段、业务含义、前端来源和核对规则 |
| [02-订单类型与交易流程.md](02-订单类型与交易流程.md) | 独立订单类型测试子模块：7 种 OrderType、ADL、Market/Limit/Stop、TP/SL，以及创建/触发/执行/取消/冻结流程 |
| [03-Fee-Funding-清算-ADL-Oracle.md](03-Fee-Funding-清算-ADL-Oracle.md) | 兼容入口；Fee、Oracle、跨域证据与四份专项文档导航 |
| [dynamicSpread-(v0.3.2).md](dynamicSpread-(v0.3.2).md) | 有符号成交点差的组成公式、影响数据、前端/Keeper 责任与边界值 |
| [Funding-(v0.3.2).md](Funding-(v0.3.2).md) | Funding 完整生命周期：参数、指数/EMA、Reader 预览、逐仓结算、领取、资金路由与边界 |
| [Liquidation-(v0.3.2).md](Liquidation-(v0.3.2).md) | 清算完整生命周期：Grace、风险判定、Keeper/前端清算价、强平执行、费用、坏账与证据 |
| [ADL-(v0.3.2).md](ADL-(v0.3.2).md) | ADL 完整生命周期：secondary 状态启停、primary 执行门槛、内部订单、严格改善、Keeper 与资不抵债 |
| [04-前端Trade操作清单.md](04-前端Trade操作清单.md) | 用户在 Trade 页面可能执行的操作与前端要求 |
| [Trade-测试用例矩阵.md](Trade-测试用例矩阵.md) | v0.3.2 交易与订单原子用例共 160 条：部署与交易基线 9 条、市价开仓与市价关仓 47 条（市价开仓 30 + 配对市价关仓 17）、市价加仓、市价部分减仓与账本 16 条、限价开仓 16 条、主单取消级联 2 条、TP/SL 与清算保护期 20 条、订单类型 16 条、其他跨域 34 条；其中代付费（Relay Fee）10 条 |
| [05-重要参数边界场景.md](../../../../Docs/contract-releases/v0.3.2/05-重要参数边界场景.md) | v0.3.2 参数边界与精确期望值来源 |
| [CMB-LONG.md](../../manual-cases/v2/CMB-LONG.md) | 保护期、TP/SL、减仓和 Standard/One-Click 组合用例 |

## 3. 文档写法

每个功能域统一回答六个问题：

1. **业务含义**：用户为什么需要它。
2. **合约行为**：使用什么字段、订单类型、价格和状态规则。
3. **影响数据**：直接改变什么，间接传导到什么，哪些只是相关而非因果。
4. **跨层要求**：合约、Keeper、Reader、SDK 和前端分别负责什么。
5. **边界值**：零值、等号、差 1、上下限、异常配置、精度与并发如何判定。
6. **测试证据**：页面、请求、交易、事件、Reader、状态和余额需要验证什么。

## 4. 结果判定词

以下状态只写入 [results.md](results.md)，不回填共享设计或版本用例正文。

- `NOT_RUN`：未在 CURRENT 登记的目标版本环境执行。
- `PASS`：步骤全部执行，断言全部成立，证据齐全。
- `FAIL`：被测功能与明确期望不一致。
- `BLOCKED`：环境或依赖不具备，记录解除条件。
- `PARTIAL`：只执行了用例中的部分层或部分断言；不得折算为 PASS。

## 5. 最小证据

每条写链用例至少保存：环境与版本、数据集、账户、orderKey、创建/执行交易哈希、区块号、事件、Reader 前后值、USDC/仓位/OI 差分。涉及 FT 时再保存页面关键截图或录屏、实际提交参数、钱包/Flash 模式、页面最终状态和错误提示。

## 6. 自动化完成定义

一个脚本只有同时满足以下条件，才可在矩阵中标记 `AUTO-COMPLETE`：

1. 测试标题能直接说明业务动作和边界，而不是只写历史用例编号。
2. 数据集固定且在报告中可见。
3. 至少存在一个交易前断言、一个执行过程断言和一个交易后断言。
4. 数值期望由公式或前态推导，不以“非空”“成功返回”代替。
5. 报告明确区分 CT、FT、XT 覆盖；未走页面时 FT 必须为 GAP。
6. 失败时能定位到功能点和断言字段，而不是只输出脚本异常。
