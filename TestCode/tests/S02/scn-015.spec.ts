...
test.describe('S02 策略下单与订单管理', () => {
  test('SCN-015 开仓同时挂括号单，TP 先触发｜LimitDecrease long @p0 @tx @serial', async ({}, testInfo) => {
    ...
    const runtime = await applyCaseTrader(loadRuntimeConfig(), 'SCN-015');
    ...
    try {
      try {
        evidence = await runMarketFlow(runtime, { scenarioId: 'SCN-015', isLong: true, closeTrigger: { orderType: 3, offsetPercent: 10 } });
      } catch (error) {
      ...
      const evidencePath = testInfo.outputPath('scn-015-evidence.json');
      await writeFile(evidencePath, stringifyEvidence(evidence), 'utf8');
      await testInfo.attach('scn-015-evidence.json', {
        path: evidencePath,
      ...
      testInfo.annotations.push({
        type: 'check-result',
        description: `链上 ${evidence.assertions.length} 项核对通过（TP 止盈触发全平（盈利瀑布））；已记录用户签名挂单/平仓、推价触发与 Keeper 执行完整回执。`,
      });
    } finally {
