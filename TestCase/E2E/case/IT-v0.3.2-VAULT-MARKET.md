...
>
> **数值纪律**：资金一律整数原始值 + 精度（USDC 6 位）；**实际结果 = 待执行**（整理日无 v0.3.2 部署与可编程 Strategy mock，06 矩阵 R11 标「fork 集成待编写」）。
>
> **锚点约定**：「文件::函数」路径相对合约仓根。

## 0. 总表

| ID | 优先级 | 域 | 来源卡片 | 环境能力 | 一句话目标 | 实际结果 |
|---|---|---|---|---|---|---|
| IT-VAULT-001 | P0 | VAULT | B32-8-01 | tx-fork（或 Foundry fork 集成）+ 可编程 Strategy mock（可指定实际转账额） | Strategy 实际到账 等于 / 少 1 / 多 1 三点，双向不等式校验 | 待执行 |
| IT-VAULT-002 | P0 | VAULT | B32-8-02 a | 同上 | 零闲置：全额 shortage 经 Strategy 校验路径到账 | 待执行 |
| IT-VAULT-003 | P1 | VAULT | B32-8-02 b | tx-fork | 足额闲置：直接转账，Strategy 不被调用 | 待执行 |
| IT-MARKET-001 | P0 | MARKET | B32-3-01 a、B32-3-02、B32-3-04 | tx-fork（CONTROLLER 可直写 DataStore；CONFIG_KEEPER / MARKET_KEEPER 角色可控） | 建市对全局 COLLATERAL_TOKEN 的零值 / 组合约束 / 写入权限三面 | 待执行 |

关联：组合场景 B32-5-04（LP 付资金费且 Vault 闲置不足）与 IT-VAULT-001 同链路，其用户侧断言并入 S08 SCN-078 扩展数据集；Relay fee token 三态（B32-3-03）与未配置时 Relay 关断（B32-3-01 b）属用户旅程，归 `../scenarios/S06-Flash异常恢复与兼容.md` 的 SCN-B32-03。
...
| 实际结果 | 待执行（无 v0.3.2 环境与 Strategy mock） |
...
| 字段 | 内容 |
|---|---|
| 来源 | 05 卡片 B32-3-01 a、B32-3-02、B32-3-04；06 矩阵 R4（`RelayCreateOrder.t.sol`/`SubaccountRelayCreateOrder.t.sol` forge 已通过；E2E 待执行；部署读值核对阻塞） |
...
1. 四条全部「待执行」：整理日无 v0.3.2 部署，tx-fork 为 base-sepolia@v0.3.1 镜像（`Docs/contract-releases/CURRENT.json`）；IT-VAULT-001/002 另需可编程 Strategy mock（06 矩阵 R11「能力待建」）。
