...
export function renderEnvironmentsHtml(generatedAt: string): string {
  ...
  const script = `
    ...
    const state={configuration:null,profile:null,marketSources:[],activeInitializationId:null,poll:null,deployBranches:[],deployBranchesLoaded:false,activeDeploymentId:null,deployPoll:null};
    ...
    async function load(){
      if(!['http:','https:'].includes(location.protocol)){
        ...
        ['save-rpc','save-config','save-profile','run-initialization','check','create-tenderly-fork','reload-market-sources','deploy-branch','reload-deploy-branches','deploy-dry-run','deploy-run'].forEach(function(id){document.getElementById(id).disabled=true;});
        return;
      }
  ...
  return renderPageShell({
    title: '测试环境',
    subtitle: '按 ①Fork 与 RPC → ②部署合约（可选）→ ③Trade/账户配置（默认折叠）→ ④Mock 初始化 → ⑤环境检查 的顺序完成配置；Mock Oracle 价格与参数直写在合约参数页',
    active: 'environments',
